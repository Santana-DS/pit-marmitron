...
class EncoderISR {
  private:
    ...
    float vel_filtrada;
    float vel_medida;
    unsigned long t_ultima_leitura = micros(); //Reduz a velocidade da leitura para que o filtro passa-baixas possa conter as oscilações da leitura
  ...
  public:
    ...
    void printLeitura(float sentido_);

    uint32_t timestamp; //Para comunicação serial
